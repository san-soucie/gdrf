...
def train_gdrf(
    ...
    log_losses = []
    pbar = trange(
        num_steps if mode is TrainingMode.OFFLINE else xs.size(-2), disable=disable_pbar
    )
    for idx in pbar:
        training_xs = xs
        training_ws = ws
        ...
        for _ in range(num_substeps):
            loss = svi.step(
                training_xs, training_ws, subsample=mode is TrainingMode.GIRDHAR
            )
        ...
        if log and idx % log_every == 0:
            ...
            if idx % (log_every * log_every_big) == 0:
                artifacts["word_topic_plot"] = wandb.Image(
                    matrix_plot(
                        np.log10(gdrf.word_topic_matrix.detach().cpu().numpy() + 1e-15),
                        title="Log Word-topic matrix",
                    )
                )
                ...
                if gdrf.dims == 1:
                    ...
                    artifacts["topic_plot"] = wandb.Image(
                        categorical_stackplot(
                            df, title="Topic Probability", label="topic"
                        )
                    )
        ...
        if early_stop:
            ...
            loss_criterion = (
                np.max(np.abs(recent_log_loss_resid)) / running_log_loss_mean
            )
