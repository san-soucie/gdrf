...
def generate_data_girdhar_thesis(K=5, V=100, N=8, W=150, H=32, R=8, eta=0.1, seed=777):
    ...
    while len(obj_topics.unique()) < K_obj and N >= K:
        obj_topics = torch.randint(0, K_obj, [N])
    obj_topics += 1
    p_v_z = torch.tensor([[1 + eta if V * k / K <= v < V * (k+1) / K else eta for v in range(V)] for k in range(K_obj)])
    ...
    for w in range(W):
        for h in range(H):
            ...
            if topics[w, h] == 0:
                ...
                words[w, h] = torch.distributions.Categorical(p_v_z[topics[w, h]-1, :]).sample()
    return centers, topics, p_v_z, words

def generate_simple_data(N=100, poisson_rate=10., seed=777, device='cpu'):
    gen = torch.Generator(device=device).manual_seed(seed)
    gen.manual_seed(seed)
    p = torch.rand(1, generator=gen, device=device)
    q = torch.rand(1, generator=gen, device=device)
    counts = [int(x) for x in torch.poisson(torch.ones(N, device=device) * poisson_rate, generator=gen)]
    out = []
    for n in range(N):
        count = counts[n]
        probs = [q, 1-q] if n < N * p else [1-q, q]
        probs = torch.tensor(probs, device=device)
        cats, obs = torch.multinomial(probs, num_samples=count, replacement=True, generator=gen).unique(return_counts=True)
        if len(cats) == 1:
...
"""
def generate_data_2d_circles(K=5, V=100, N=8, W=150, H=32, R=8, eta=0.1, seed=777, device='cpu', permute=False, constant_background=True):
    K_obj = K - 1
    gen = torch.Generator(device=device).manual_seed(seed)
    gen.manual_seed(seed)
    ...
    xs = torch.reshape(torch.stack(torch.meshgrid(torch.tensor(list(range(W)), device=device), torch.tensor(list(range(H)), device=device))), (2, W*H)).T
    counts = torch.randint(low=V, high=10*V, size=(W*H,), device=device, generator=gen)

    while len(obj_topics.unique()) < K_obj and N >= K:
        obj_topics = torch.randint(0, K_obj, [N], device=device, generator=gen)
    obj_topics += 1
    K_obj_probs = K_obj if constant_background else K
    p_v_z = torch.tensor([[1 + eta if V * k / K_obj_probs <= v < V * (k+1) / K_obj_probs else eta for v in range(V)] for k in range(K_obj)], device=device)
    ...
    if constant_background:
        ...
        bg_dist = torch.Tensor([1 + eta if V * (K-1) / K <= v <= V else eta for v in range(V)]).to(probs.device)
    ...
    for idx in range(len(counts)):
        cats, obs = torch.multinomial(input=probs[idx, :], num_samples=counts[idx], replacement=True, generator=gen).unique(return_counts=True)
...
def mutual_information(hgram: torch.Tensor, dim: int = 0):
    # https://matthew-brett.github.io/teaching/mutual_information.html
    pxy = hgram.div(hgram.sum())
    #pxy = hgram.normalize(p=1, dim=dim, keepdim=True, dtype=torch.get_default_dtype())
    px = pxy.sum(1, keepdim=True)
    py = pxy.sum(0, keepdim=True)
    px_py = px.mul(py).unsqueeze(dim=0)
    nzs = pxy > 0
    return pxy[nzs].mul(pxy[nzs].div(px_py[nzs]).log()).sum()

def jitter(Kff, N, jitter, maxjitter):
    njitter = 0
    Lff = None
...
def dirichlet_param(b: torch.Tensor, K: int, V: int, device: str = 'cuda') -> torch.Tensor:
...
#     svi = pyro.infer.SVI(model, guide, optimizer, objective)
#
#     losses = []
#     log_losses = []
...
#
#         if log:
#             p = gdrf.perplexity_tensor(x, w).item()
#             gdrf.log_wandb(epoch=idx,
#                            loss=loss,
#                            perplexity=p,
